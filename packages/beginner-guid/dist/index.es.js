...
const getPosition = (selector) => {
  ...
  if (window.innerWidth - right < dw + offset) {
    positionX = "right";
    x2 = window.innerWidth - left + offset;
    if (srcX < dw + offset) {
      positionX = "left";
      x2 = srcX;
      positionY = "top";
...
const generateClipPathStr = (x2, y, w2, h2) => {
  const fullWith = window.innerWidth;
  const fullHeight = window.innerHeight;
  const wx = x2 + w2;
  const hy = y + h2;
  const line1 = `M 0 0 H 0 V ${fullHeight} H ${x2} V ${fullHeight} H${x2} V 0 Z`;
  const line2 = `M ${x2} 0 H ${x2} V ${y} H ${fullWith} V ${y} H ${fullWith} V ${hy} H ${fullWith} V 0 Z`;
  const line3 = `M ${x2} ${hy} H ${x2} V ${fullHeight} H ${fullWith} V ${fullHeight} H ${fullWith} V ${hy} Z`;
...
let YoungBeginnerGuidNext = class extends s {
  ...
  render() {
    ...
    <div id="mask"></div>
    <div id="dialog">
      ${!this.handler.force ? $`<div id="dialog-close" title="关闭新手引导" @click="${this.closeHandler}">
...
YoungBeginnerGuidNext.styles = r$2`
    ${o$3(cssContent)}
  `;
__decorateClass([
  e()
...
YoungBeginnerGuidNext = __decorateClass([
  n$1("young-beginner-guid-next")
], YoungBeginnerGuidNext);
class YoungBeginnerGuidControllerNext {
